...
class ConventionControllerTest {
	...
	@Test
	void conventionAppliedResourcesExists() throws Exception {
		final String content = """
				{
				  "apiVersion": "conventions.apps.tanzu.vmware.com/v1alpha1",
				  "kind": "PodIntent",
		...
		this.mockMvc.perform(post("/")
				...
				.andExpect(jsonPath("$.status.template.spec.containers").isArray())
				.andExpect(jsonPath("$.status.template.spec.containers.length()").value(1))
				.andExpect(jsonPath("$.status.template.spec.containers.[0].resources").isNotEmpty())
				.andExpect(jsonPath("$.status.template.spec.containers.[0].resources.limits").isNotEmpty())
				.andExpect(jsonPath("$.status.template.spec.containers.[0].resources.requests").isNotEmpty())
				.andExpect(jsonPath("$.status.template.spec.containers.[0].resources.limits.memory").value("256Mi"))
				.andExpect(jsonPath("$.status.template.spec.containers.[0].resources.requests.memory").value("256Mi"))
				.andExpect(jsonPath("$.status.appliedConventions").isArray())
				.andExpect(jsonPath("$.status.appliedConventions.length()").value(1))
				.andExpect(jsonPath("$.status.appliedConventions[0]").value("add-env-var"));
	}
